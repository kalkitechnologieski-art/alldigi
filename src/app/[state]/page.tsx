import { getStateMeta, getCitiesByState } from '@/lib/data';
...
export default async function StatePage({ params }: { params: { state: string } }) {
    ...
    return (
      <main className="max-w-4xl mx-auto py-12 px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-8">
          {countries.map(c => (
            <Link key={c.slug} href={`/global/${c.slug}`} className="p-4 bg-gray-100 rounded-lg hover:bg-blue-50">{c.name}</Link>
          ))}
        </div>
      </main>
    );
  ...
  return (
    <main className="max-w-6xl mx-auto py-12 px-4">
      <h1 className="text-3xl font-bold mb-6 capitalize">Digital Marketing Agencies in {stateMeta.displayName}</h1>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {cities.map(c => (
          <Link key={c.slug} href={`/${params.state}/${c.slug}`} className="p-4 bg-gray-50 rounded-lg hover:shadow transition">
            <h2 className="text-lg font-semibold">{c.name}</h2>
            <p className="text-sm text-gray-500">{c.agencyCount} localities</p>
          </Link>
        ))}
      </div>
    </main>
